    ...
    if [ "$skip" == false ]; then
    	...
    	for num_experts in ${num_experts_options[@]}; do	    
			for sample_method in ${sample_method_options[@]}; do
				...
				expert_file_path="${SCRATCH}/code/moose/outputs/hub_selection_10/${sample_method}_top${num_experts}/${task}.txt"
				#if [ ! -f "${expert_file_path}" ]; then
				#	mkdir -p ${SCRATCH}/code/moose/outputs/hub_selection/${sample_method}_top${num_experts}
				#	#echo "r-three/lora_baseline_lr3e-4_step400_rank64_${task}" > ${expert_file_path} && \
				#	echo "shared_space/lora_baseline_10_lr3e-4_step100_rank64_${task}" > ${expert_file_path} && \
				#	head -n -1 ${SCRATCH}/code/moose/outputs/hub_selection/${sample_method}_top${num_experts}_wo_target/${task}.txt >> ${expert_file_path}
				#fi

				# if include_best_experts is false, drop the first line
				if [ "$include_best_lora" == "False" ]; then
					expert_file_path="${SCRATCH}/code/moose/outputs/hub_selection_10/${sample_method}_top${num_experts}_wo_target/${task}.txt"
				fi
